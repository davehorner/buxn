...
static void
buxn_audio_start(buxn_vm_t* vm, buxn_audio_t* c, uint8_t* mem) {
	...
	c->len = buxn_audio_load2(mem + 0xa);
	if(c->len > 0x10000 - addr)
		c->len = 0x10000 - addr;
	c->addr = &vm->memory[addr];
	c->volume[0] = mem[0xe] >> 4;
	c->volume[1] = mem[0xe] & 0xf;
	c->repeat = !(mem[0xf] & 0x80);
	if(pitch < 108 && c->len)
...
buxn_audio_deo(struct buxn_vm_s* vm, buxn_audio_t* device, uint8_t* mem, uint8_t port) {
	if(port == 0xf) {
		buxn_audio_lock_device();
		buxn_audio_start(vm, device, mem);
		buxn_audio_unlock_device();
	}
}
